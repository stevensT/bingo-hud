...
///
/// <para>
/// One vocabulary for the whole app. The panel has phrased ages since 6.8 and the HUD needs the
/// same words at 7.1 to mark a stale reading; leaving each surface to phrase its own is how a
/// panel saying "21 min old" ends up beside a HUD saying "21m", which reads as two readings
/// rather than one.
/// </para>
/// <para>
...
{
    /// <summary>
    /// The age as a label, to sit beside a number: "just now", "21 min old", "3 hours old".
    /// </summary>
    public static string Old(TimeSpan age) =>
        age < TimeSpan.FromMinutes(1) ? "just now" : $"{Span(age)} old";

    /// <summary>
    /// The age as a bare duration, to sit inside a sentence: "21 min", "3 hours".
    ///
    /// <para>
