...
The task carried no acceptance criteria and the spec never mentions a tray, so the menu was built
from what the spec asks for elsewhere and has no other route to. Collapse (AC-7) and display
direction (AC-2a) are both required user settings that persist (AC-22), and until now neither
could be changed without editing the settings file by hand. Opening the panel is there too,
because a HUD parked under another window is hard to click and a tray icon never is. Quit is
there because the HUD is frameless by design and nothing else could end the app.

Deliberately left out: manual refresh and mute. AC-28 places manual refresh in the detail panel,
not the tray, and no task has placed that control — see the gap noted below. Mute (AC-18) waits
for 6.10, because nothing raises an alert yet and a menu item with no observable effect is a
trap.

`NotifyIcon` from Windows Forms rather than hand-written `Shell_NotifyIcon`. Windows offers no
WPF control for the notification area, and the alternatives were a NuGet dependency or several
hundred lines of interop with a message-only window to receive its callbacks. The cost is
`UseWindowsForms`, which adds implicit usings for `System.Windows.Forms` and `System.Drawing`
that collide with the WPF types of the same names — `Application`, `Brush`, `Color`, `Point`,
`Size`. Both are removed in the csproj so the WPF meaning wins everywhere, and the two borrowed
types are named in full at the one place they are used. Weigh the added assemblies when BV.3
settles the publish shape.

The icon is generated by `scripts/make-icon.ps1` rather than committed as an opaque binary, so it
can be regenerated and argued with. It is a ring gauge, three quarters filled, which survives 16
px in a way a letter does not. Two things were learned making it. It first drew white on
transparent, which is invisible on a light Windows 11 taskbar, so there is now a dark halo under
the ring that costs nothing on a dark taskbar and is what makes it legible on a light one. And
the frames were first written as PNG-in-ICO, which WPF reads and `System.Drawing` does not —
the tray needs a `System.Drawing.Icon`, so a PNG-only icon renders in the notification area as
noise. Frames up to 128 px are now uncompressed; 256 stays PNG, which the tray never asks for.

Verified by driving the real shell: launch the exe, open the hidden-icons flyout through UI
Automation, right-click the icon, photograph the menu, click "Collapse to one line", and read the
settings file back. The menu showed all five entries in order and the file came back with
collapse set. Trevor's own settings file was deleted at the start of that run and rewritten, so
the HUD position saved before 6.9 is gone; collapse was set back to false afterwards.

Found while verifying: the HUD was taking a taskbar button. Harmless before, wrong once a tray
icon exists — an always-on-top widget should live in the tray or the taskbar, not both. Now
`ShowInTaskbar="False"`, which also means the tray icon is the only way to quit.

Also found, and it is a user-facing fact rather than a defect: Windows 11 places every new
notification-area icon in the hidden overflow. Bingo's icon is registered and works, but a user
will not see it until they drag it out of the flyout or promote it in Settings. An app cannot
opt out of this. It belongs in the README's setup notes, which is Trevor's call.

Gap in the task list, raised rather than filled: AC-28 says a manual refresh is available from
the detail panel and says what it must report when refused. The behaviour is met and tested in
Core at 4.9, but no task in Phase 6 places the control, and 6.8's list did not mention it. As
things stand the criterion cannot be met by 6.12.
